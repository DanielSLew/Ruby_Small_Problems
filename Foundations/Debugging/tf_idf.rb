# the following is a simple implementation of TF-IDF (a method of information extraction)
# This is supposed to work as a basic search engine. Fix the following code
...
# After some investigating in pry what each method was returning I realized that on line 28, the method Math.log was returning 0
# when it shouldn't have. And this is because when the number being passed to Math.log as an argument was
# 1 when it should have been more than 1. This issue can be fixed by calling the method Integer#to_f on one of the
# numbers in Math.log's argument. This give us a decimal number which then gives us the correct number on 
# line 10.

# On line 28 we've added a condition that returns 0 if number_of_documents_with_term == 0
# This is because on the next line we use that Integer variable as a divisor, and using a divisor that equals 0 gives you infinity.
# Having this return condition protects against cases that will cause the program to return NaN.